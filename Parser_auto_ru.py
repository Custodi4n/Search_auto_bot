...
def get_content(html: bytes) -> List[dict]:
    ...
    for item in items:
        car = {
            'car model': item.find('a', 'Link ListingItemTitle__link').get_text(),
            'engine': item.find('div', 'ListingItemTechSummaryDesktop__cell').get_text(),
            'url': item.find('a', 'Link ListingItemTitle__link').get('href'),
            'price': item.find('div', 'ListingItemPrice__content').get_text().replace(' ', '').replace('₽', '').replace('от', ''),
            'year': item.find('div', 'ListingItem__yearBlock').get_text(),
            'km': item.find('div', 'ListingItem__kmAge').get_text(),
            'city': item.find('span', 'MetroListPlace__regionName MetroListPlace_nbsp').get_text(),
        }
        cars.append(car)
    return cars
...
def save_to_file(data) -> None:
    ...
    with open(PATH, 'w', newline='', encoding='utf-8') as file:
        ...
        w.writerow(['Car', 'Engine', 'Link', 'Price', 'Year', 'Km', 'City'])
        for car in data:
            w.writerow([
                car['car model'],
                car['engine'],
